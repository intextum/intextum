...
def get_app_version() -> str:
    ...
    if env_value:
        return env_value
    file_value = _read_version_file()
    if file_value:
        return file_value
    return _read_distribution_version() or _FALLBACK_VERSION


def _read_distribution_version() -> str | None:
    """Return the installed wheel's version, if the package is pip-installed.

    A ``VERSION`` file sits next to the source in the repo and in the Docker
    image, but not in ``site-packages`` when installed from a wheel; fall back
    to the distribution metadata in that case.
    """
    try:
        from importlib.metadata import PackageNotFoundError, version

        return version("intextum-worker")
